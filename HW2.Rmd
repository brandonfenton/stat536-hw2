...
word_document:
  fig_height: 7
  fig_width: 10
author: "Brandon Fenton and Kenneth Flagg"
output: pdf_document
...
bozetmp$FYEAR <- paste(bozetmp$YEAR + 1900, "+", bozetmp$MONTH, "/12", sep="")

bozetmp$MONTH <- as.factor(bozetmp$MONTH)

bzt.fit <- lm(MMXT~ YEAR + MONTH, data=bozetmp)

pander(bzt.fit, move.intercept=FALSE)
```
...
bzt.fit2 <- lm(MMXT~ YEAR + MONTH : YEAR:MONTH, data=bozetmp)
...
Tukey_results<-glht(bzt.fit,linfct=mcp(MONTH="Tukey"))
...
bzt.fit3 <- lm(MMXT~ YEAR + MONTH + YEAR^2, data=bozetmp)
pander(bzt.fit3)
```
